...
<html>
         ...
         <body>
                  ...
                  <div id="categories_history">   
                           <a href="viewHistory" class="ctgr" >View History</a>
                           <a href="searchHistoryPage" class="ctgr" style="border-bottom: 3px solid black;background: #F5A9BC;">Search History</a>
                  </div>
                  <form action="searchHistory" id="search_history_form">
                           <font id="login"><b>Search History</b></font><br><br>
                           <b>Subject Name: </b><input type="text" name="txtSearchHistory" value="${param.txtSearchHistory}" id="search_history" />
                  ...
                  <c:if test="${not empty param.txtSearchHistory || not empty param.txtDate}">
                           <c:if test="${empty requestScope.DATE_ERROR}">
                                    <c:if test="${empty requestScope.HISTORY}">
                                             <div id="invalid1">
                                                      <h1>No history matches!!!! <i class="far fa-laugh-squint"></i></h1><br>
                                             </div>
                                    </c:if>
                           </c:if>
                  </c:if>    
                  <div id="paging">
                           ...
                           <c:if test="${(pageNumber lt totalPage) && (pageNumber gt 0)}">
                                    <c:url var="last" value="searchHistory">
                                    ...
                                    <a href="${last}" ><i class="fas fa-angle-left" id="paging" ></i></a><span style="font-size: 13px">${pageNumber+1} / ${totalPage+1}</span>
                                             <c:url var="next" value="searchHistory">
                                                      <c:param name="txtDate" value="${param.txtDate}"/>  
                                                      <c:param name="txtSearchHistory" value="${param.txtSearchHistory}"/> 
                                                      <c:param name="txtPageNumber" value="${pageNumber+1}"/>
                                             </c:url>
